...
var helpers = {
  ...
  slideHandler: function (index, sync, dontAnimate) {
    ...
    this.setState({
      ...
      ReactTransitionEvents.addEndEventListener(this.refs.track.getDOMNode(), this.slideEndedAnimationEnded);
    });
  },

  slideEndedAnimationEnded: function () {
    ReactTransitionEvents.removeEndEventListener(this.refs.track.getDOMNode(), this.slideEndedAnimationEnded);
    this.setState({
      animating: false,
      trackStyle: this.getCSS(this.state.currentLeft),
      swipeLeft: null
    });
    if (this.props.slideEndedCallback) {
      this.props.slideEndedCallback(this.state.currentSlide);
    }
  },
